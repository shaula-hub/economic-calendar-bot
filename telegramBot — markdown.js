...
class EconomicCalendarTelegramBot {
  ...
  // In telegramBot.js, update the escapeMarkdown function:
  escapeMarkdown(text, parseMode = "MarkdownV2") {
    if (!text) return text;
    ...
    // For classic Markdown, only escape these
    return text.replace(/([_*\[\]`])/g, "\\$1");
  }
  ...
  async checkChannelSubscription(bot, userId, channelUsername) {
    try {
      ...
      // TEMPORARY FIX: If bot is not admin in channel, allow access
      if (error.response && error.response.error_code === 400) {
        console.warn(
          `⚠️ Bot is not admin in channel ${channelUsername}. Allowing access temporarily.`
        );
        return true; // TEMPORARY - change to false when bot is admin
      }
...
`.trim();
  ...
  async handleStart(msg) {
    ...
    // Remove the congratulation logic - it's not working correctly
...
`.trim();
  ...
  async handleSubscribe(msg) {
    ...
    try {
      // This will set is_active to true automatically
...
📊 Вы будете получать график планируемых экономических событий каждый день в 05:00.
...
💡 Не забудьте подписаться на наш канал ${channelUrl} для получения дополнительных материалов!`;
  ...
  async handleUpdateData(msg) {
    ...
    try {
      // Send initial update message with HTML formatting
      await this.bot.sendMessage(
        chatId,
  ...
  async handleStatus(msg) {
    ...
    try {
      ...
      // Fix the status display - check is_active for bot subscription status
      const botSubscriptionStatus = user.is_active
...
Рассылка бота: ${botSubscriptionStatus}
